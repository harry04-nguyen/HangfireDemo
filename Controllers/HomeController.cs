...
namespace HangfireDemo.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            var jobId = _backgroundJobClient.Enqueue<IJobService>(x =>
                x.SimpleJob("Fire-and-forget job triggered!")
            );
        ...
        public IActionResult EnqueueJobWithRetry()
        {
            ...
            TempData["Message"] =
                $"Job with retry enqueued with ID: {jobId}. This will fail initially and retry.";
            return RedirectToAction("Index");
        }
        ...
        public IActionResult EnqueueDelayedJob()
        {
            ...
            TempData["Message"] =
                $"Delayed job scheduled with ID: {jobId}. Will execute in 2 minutes.";
            return RedirectToAction("Index");
        }
        ...
        public IActionResult EnqueueLongRunningJob()
        {
            ...
            TempData["Message"] =
                $"Long-running job enqueued with ID: {jobId}. Will run for 10 seconds.";
            return RedirectToAction("Index");
        }
        ...
        {
            var parentJobId = _backgroundJobClient.Enqueue<IJobService>(x =>
                x.SimpleJob("Parent job")
            );
            ...
            TempData["Message"] =
                $"Parent job ID: {parentJobId}, Continuation job ID: {continuationJobId}";
            return RedirectToAction("Index");
        }
        ...
        public IActionResult EnqueueBatchJobs()
        {
            ...
            {
                var jobId = _backgroundJobClient.Enqueue<IJobService>(x =>
                    x.SimpleJob($"Batch job #{i}")
                );
                jobIds.Add(jobId);
            }

            TempData["Message"] =
                $"Enqueued {jobIds.Count} batch jobs. Check dashboard for details.";
            return RedirectToAction("Index");
        }
