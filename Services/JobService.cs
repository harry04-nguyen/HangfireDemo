...
public JobService(ILogger<JobService> logger)
  {
   ...
   _logger.LogError($"[JobWithRetry] Simulating failure on attempt #{attemptNumber}");
              throw new InvalidOperationException($"Simulated failure on attempt {attemptNumber}");
            }

       await Task.Delay(500);
            _logger.LogInformation($"[JobWithRetry] Successfully completed on attempt #{attemptNumber}");
    }

        public async Task LongRunningJob(int durationInSeconds)
        {
          _logger.LogInformation($"[LongRunningJob] Starting long-running job for {durationInSeconds} seconds at {DateTime.Now}");
            
            for (int i = 1; i <= durationInSeconds; i++)
            {
       await Task.Delay(1000);
                _logger.LogInformation($"[LongRunningJob] Progress: {i}/{durationInSeconds} seconds elapsed");
            }
  ...
  {
            _logger.LogInformation($"[DataProcessingJob] Started processing data at {DateTime.Now}");
 ...
 // Simulate data processing
  ...
  await Task.Delay(1000);
 _logger.LogInformation($"[DataProcessingJob] Data persisted to database at {DateTime.Now}");
        }
